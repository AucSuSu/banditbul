...
const Map: React.FC = () => {
  ...
  const [deleteSelectEdge, setDeleteSelectEdge] = useState<number | null>(null);
  ...
  const [mapImgaeUrl, setMapImageUrl] = useState<string>(
    "https://d3h25rphev0vuf.cloudfront.net/싸피역-1.png"
  );
    ...
    ws.current.onmessage = async (event: MessageEvent) => {
      ...
      if (d.type === "MONITOR" && d.count) {
        ...
        if (!result) {
          ...
          setBeacons((prev) => [
            sosBeacon[0],
            ...prev.filter((e) => e.beaconId !== sosBeacon[0].beaconId),
          ]);
        }
  ...
  const handleRadioChange = (beaconId: string) => {
    ...
    if (isIdNotPresent) {
      ...
      alert(beaconId);
      const newBeacon = beacons.find((item) => item.beaconId === beaconId);
  ...
  const handleBeaconDeleteConfirm = async () => {
    ...
    if (deleteSelectBeacon != "-1") {
      try {
        console.log("삭제");
        const response = await axios.delete(`/beacon/${deleteSelectBeacon}`);
        const data = response.data.object;
        console.log(data);
        getMapInfo(floor);
      } catch (error) {
  ...
  const handleEdgeDeleteConfirm = async () => {
    ...
    if (deleteSelectEdge != -1) {
      try {
        console.log("삭제");
        const response = await axios.delete(`/edge/${deleteSelectEdge}`);
        const data = response.data.object;
        console.log(data);
        getMapInfo(floor);
      } catch (error) {
        ...
        <div className={styles.contentContainer}>
          <div className={styles.leftBackground}>
            <div className={styles.leftContainer}>
              ...
              {!modalshow && <EdgeToggle state={state} setState={setState} />}
              <div
                className={styles.beaconList}
                style={{
                  height: modalshow ? "100%" : "80%",
                  backgroundColor: modalshow ? "white" : "transparent",
                  boxShadow: modalshow ? "0px 4px 16px rgba(0, 0, 0, 0.2)" : "",
                }}
              >
                {modalshow ? (
                  <>
                      ...
                      <div className={styles.titleBox}>
                        ...
                        <div className={styles.addBeaconDes}>
                          ...
                          <div
                            ...
                            style={{ color: "black" }}
                          >
                            우측 반딧불 아이콘을 원하는 위치에 드래그하세요!{" "}
                            <br />
                            모든 위치는 바라보는 방향 기준으로 지정해주세요.
                          </div>
                        </div>
                      </div>

                      {Options(selectType, x, y, floor, closeAddModal)}
                    </div>
                  </>
                ) : (
                    ...
                    {state ? (
                        ...
                        {beacons.map((item, index) =>
                          sosBeaconIdList.has(item.beaconId) ? (
                            <div
                              ...
                              key={index}
                              onMouseOver={() =>
                                beaconHandleMouseOver(item.beaconId)
                              }
                            >
                              <div className={styles.sosBeaconContent}>
                                <p className={styles.sosBeaconTitle}>
                                  SOS 신호가 발생되었습니다
                                  <br />
                                  {/* 위치를 확인하고 도움을
                                                        주세요 */}
                              ...
                              <div className={styles.yesNoContainer}>
                                <span
                                  className={styles.sosAcceptButton}
                                  onClick={() => {
                                    sendAcceptMessage(item.beaconId);
                                  }}
                                >
                                  수락
                                </span>
                            ...
                            <>
                              <div
                                ...
                                onMouseOut={beaconHandleMouseOut}
                              >
                                {" "}
                                <div className={styles.deleteModalContent}>
                                  <p className={styles.deleteDes}>
                                    삭제하시겠습니까?
                                  </p>
                                  <div className={styles.yesNoContainer}>
                                    <span
                                      onClick={handleBeaconDeleteConfirm}
                            ...
                            >
                              {addEdgeState && (
                                <input
                                  ...
                                  checked={selectedEdges.some(
                                    (data) => data.beaconId === item.beaconId
                                  )}
                                />
                              )}
                              <div className={styles.beaconId}>
                                {typeToKor(item.beaconTYPE!)}
                              </div>
                              ...
                              <img
                                src={IconDelete}
                                alt=""
                                className={styles.beaconDeleteButton}
                                onClick={() => {
                                  openModal();
                                  setDeleteSelectBeacon(item.beaconId);
                                }}
                              />
                            </div>
                          )
                      ...
                      <>
                        {edgeList.map((item, index) =>
                          deleteSelectEdge == item.edgeId ? (
                            <>
                              <div
                                className={styles.beaconListItem}
                                key={index}
                                onMouseOver={() =>
                                  edgeHandleMouseOver(item.edgeId)
                                }
                                onMouseOut={edgeHandleMouseOut}
                              >
                                <div className={styles.deleteModalContent}>
                                  <p className={styles.deleteDes}>
                                    삭제하시겠습니까?
                            ...
                            <>
                              ...
                              >
                                <div className={styles.beaconId}>
                                  {typeToKor(item.beacon1Type!)}-
                                  {typeToKor(item.beacon2Type!)}
                                </div>
                                <img
                                  src={IconDelete}
                                  alt=""
                                  ...
                                  onClick={() => {
                                    openModal();
                                    setDeleteSelectEdge(item.edgeId);
                                  }}
                                />
                              </div>
                            </>
                ...
                <div className={styles.picContainer}>
                  {picIcons.map((data, index) => (
                    <div className={styles.picItem} key={index}>
                      <img className={styles.pic} src={data.img} alt="" />
                      <div className={styles.typeName}>{data.type}</div>
                    </div>
                  ))}
                </div>
                    ...
                    return (
                      <line
                        ...
                        style={{
                          height: "1px",
                          stroke:
                            edge.edgeId == clickedEdge ? "#6565ff" : "black",
                          strokeWidth: 3,
                        }}
                      />
                    );
...
function Options(
  ...
  switch (type) {
    case 1:
      return <Toilet x={x} y={y} floor={floor} closeModal={closeAddModal} />;
    case 2:
      return <Gate x={x} y={y} floor={floor} closeModal={closeAddModal} />;
    case 3:
      return <Exit x={x} y={y} floor={floor} closeModal={closeAddModal} />;
    case 4:
      return <Stair x={x} y={y} floor={floor} closeModal={closeAddModal} />;
    case 5:
      return <Elevator x={x} y={y} floor={floor} closeModal={closeAddModal} />;
    case 6:
      return <Escalator x={x} y={y} floor={floor} closeModal={closeAddModal} />;
    case 7:
      return (
        <ScreenDoor x={x} y={y} floor={floor} closeModal={closeAddModal} />
      );

    case 8:
      return <Point x={x} y={y} floor={floor} closeModal={closeAddModal} />;
  }
}
